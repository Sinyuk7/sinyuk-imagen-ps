...
- 当前先写业务核心，再写 CLI，再写 UI。
- 当前先建立可复用的命令入口，再接具体 surface。
- Photoshop 仍然是第一目标 host，但不要求在当前 change 内先完成 UI。
- UI 未来可以同进程直接调用 shared commands，不需要通过外部 CLI 进程绕行。

## 当前仓库真实状态

> 本章节已收敛到根级 `STATUS.md`，请以 `STATUS.md` 的模块级完成状态为准。

### 已验证

- 2026-04-23 本地执行 `pnpm build` 通过
- 2026-04-23 本地执行 `pnpm test` 失败；原因是当前多个 package 尚无测试文件，`vitest run` 以 `No test files found` 退出
...
3. settings / config / submit / retry 这类用例是否能通过 shared commands 统一暴露
...
| 层 | 负责什么 | 不负责什么 |
|---|---|---|
| `packages/core-engine` | job lifecycle、workflow orchestration、provider dispatch、event emission、runtime API | UI、CLI 命令、Photoshop API、文件系统、provider 参数语义 |
...
| `shared commands` | 面向 surface 的轻量命令入口、依赖组装、统一返回结构 | runtime 状态机、provider 参数语义、直接 IO |
| `CLI` | 命令解析、参数输入输出、面向开发和自动化的入口 | 业务核心、provider 内部语义、runtime 内部状态 |
| `app` | 当前唯一应用目录，未来承接 Photoshop UI 与 host integration | 当前 change 的成功标准、runtime orchestration、provider semantics |

## Shared Commands 约束

当前计划明确引入 shared commands，但这层必须保持极薄。
...
- 不拥有自己的状态机
- 不复写 runtime lifecycle
- 不解释 provider 参数语义
- 不直接做网络或文件 IO
- 不长成 UI 专属 view model
...
- 首个 surface 是 `CLI`
- CLI 用来验证 shared commands 是否足够稳定
- 不要求 UI 在本阶段接入

### 后续阶段

- UXP UI 直接同进程调用 shared commands
- MCP / skill 未来也复用这套 shared commands
...
- 明确 shared commands / CLI / UI 的职责边界
...
- `submitJob` / `getJob` / `subscribe` 等最小 runtime API
...
- shared commands 可以独立驱动 runtime 与 provider
- shared commands 不复写 runtime 状态机
- shared commands API 足够供 CLI 使用
...
- shared commands contract tests
- CLI smoke tests
...
- UXP 主页面
- settings page
- provider config local persistence adapter 与 UI 接入
- asset adapter 与 UI 接入

后续约束：

- UI 直接复用 shared commands
...
3. provider 配置可以通过 adapter 持久化，并通过 shared commands 暴露。
4. shared commands 能统一暴露 provider 与 job 相关命令。
5. CLI 能完成 provider 配置、任务提交、状态查询和失败重试。
6. 共享层边界没有 host API 泄漏，也没有 UI 语义倒灌进 runtime。
...
- shared commands 放在哪个 package 或目录最合适？
- CLI 目录采用 `apps/cli` 还是 `packages/cli` 的形式？

默认处理原则：

- 能不阻塞 runtime 主链路的，先不要放进当前 gate
- 能由 shared commands 收敛的 surface 差异，不要拉进 runtime
- 能由 adapter 吞掉的 IO 差异，不要拉进 shared commands

## 文档维护规则

- 当前 change 的范围、阶段、验收标准，只看本文件。
- `DESIGN.md`、`TOKEN.md`、`UI_MAIN_PAGE.md` 仍然有效，但属于后续 UI 阶段参考，不属于当前 gate。
- 每完成一个 phase，都要同步更新根级 `STATUS.md` 中的模块完成状态。
