...
- When a portaled popup overlaps the shared composer `UxpTextArea`, Photoshop UXP can still route clicks to the native textarea even if the popup is visually above it. Treat this as a textarea hit-test bug first: suspend textarea pointer events and blur it while the popup is open instead of reworking the menu click handlers again.
