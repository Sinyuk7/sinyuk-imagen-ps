...
.section-title{ font-size:11px; font-weight:600; color:var(--app-color-text-secondary); letter-spacing:.4px; margin-bottom:12px; }
...
.prompt-preset-row.is-selected{
  background:var(--app-color-accent-soft);
  border-left-color:var(--app-color-accent-default);
}
...
.model-config-ratio-visual{
  display:block;
  border:1px solid currentColor;
  border-radius:4px;
  opacity:.92;
}
.model-config-ratio-visual.is-wide{
  width:22px;
  height:12px;
}
.model-config-ratio-visual.is-standard{
  width:18px;
  height:14px;
}
.model-config-ratio-visual.is-square{
  width:15px;
  height:15px;
}
.model-config-ratio-visual.is-tall{
  width:12px;
  height:22px;
}
.model-config-ratio-visual.is-generic{
  width:16px;
  height:16px;
}
...
.scroll-footer-pad-detail{ padding-bottom:80px; }
