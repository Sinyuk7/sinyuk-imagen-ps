...
- 更新时间：2026-06-16
- 接入设计：见 [INTEGRATION_DESIGN.md](INTEGRATION_DESIGN.md)
...
- 非 UXP 环境 fallback 到 in-memory storage / host stub，便于 build/test。
...
| `pages/history-page.tsx` | 历史页 | session-local conversation rounds |
...
- `uxp-provider-profile-repository.ts`：UXP data folder JSON profile repository，非 UXP fallback 到 in-memory。
- `uxp-secret-storage-adapter.ts`：UXP secureStorage secret adapter，非 UXP fallback 到 in-memory。
...
| History | session-local | 若需要持久历史，先在 application/session 与 core-engine 边界设计 `listJobs()` 或持久 job store |
