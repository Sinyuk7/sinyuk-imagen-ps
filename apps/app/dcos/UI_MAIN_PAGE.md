...
主页面 header 左侧提供历史图标入口，点击进入独立的历史任务页。

### 页面定位

- 列出所有历史任务回合，时间倒序，按日期分组（今天 / 昨天 / 更早）
- 解决对话区过长导致历史难以检索的问题
- 不替代主页面的对话流，只是补充检索入口

### 页面结构

1. **Header** — 返回按钮（回到主页面）+ "历史任务" 标题
2. **Filter chips** — 全部 / 已完成 / 运行中 / 失败，水平滚动
3. **任务列表** — 按日期分组，每组有 sticky 日期标签

### 任务行结构

| 元素 | 说明 |
|------|------|
| 缩略图 / 状态图标 | 成功：图像缩略图；运行中：橙色旋转图标；失败：红色 × |
| prompt 文字 | 2 行截断，超出省略 |
| provider · 时间 | 次要信息行 |
| 状态标签 | 带颜色圆点 + 文字（完成 / 生成中 / 失败） |
