...
import { fakeProfile, createFakeServices } from '../../../helpers/fakes';
...
describe('SettingsDetailPage contract — profile editing', () => {
  ...
  it('keeps detail-page alias stable while normalizing a gemini full URL through the shared import model', async () => {
    ...
    await act(async () => {
      changeInput(
        queryByTestId(container, 'provider-endpoint-url-0'),
        'https://grsai.dakka.com.cn/v1beta/models/nano-banana-fast:generateContent',
      );
    });

    expect((queryByTestId(container, 'provider-alias-input') as HTMLInputElement).value).toBe('Existing Alias');
    expect((queryByTestId(container, 'provider-endpoint-url-0') as HTMLInputElement).value).toBe('https://grsai.dakka.com.cn/v1beta');
