...
import { listProfileModels } from '@imagen-ps/application';
 ...
 *
 * INTENT: 暴露 `listProfileModels` 的候选来源链（cache → impl default → empty）查询能力。
...
export function registerProfileModelsCommand(profile: Command): void {
  profile
    ...
    .description('List effective candidate models for a profile (cache → implementation default → empty)')
