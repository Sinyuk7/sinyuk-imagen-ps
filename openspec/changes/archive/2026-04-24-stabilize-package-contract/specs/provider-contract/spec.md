...
定义 `packages/providers` 的稳定公开契约，使后续 registry、mock provider、openai-compatible provider 和 transport 实现都建立在同一套 provider contract 之上，并保持与 `@imagen-ps/core-engine` 的边界清晰。
