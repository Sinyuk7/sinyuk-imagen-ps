...
`retryJob(jobId: string)` SHALL 重试指定的 job。内部 SHALL 从 store 获取原 job 的 `workflowName` 和 `input`，然后调用 `runtime.runWorkflow` 创建新 job。返回类型 SHALL 为 `Promise<CommandResult<Job>>`。
...
#### Scenario: 重试不存在的 job
- **WHEN** 调用 `retryJob('nonexistent-id')` 且该 jobId 不存在
