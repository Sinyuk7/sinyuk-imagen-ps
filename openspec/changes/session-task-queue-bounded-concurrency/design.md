...
- core runtime 提供 host-neutral 的可选 `onJobCreated` handoff，并在进入 workflow execution 前等待它完成；session scheduler 用该 seam 把真实 `jobId` 写入 running `TaskRecord`，不把 queue ownership 下沉到 core。
...
| `packages/core-engine` runtime / store | Extend | 继续用 `runtime.runWorkflow()` 创建执行 job，不扩展 `JobStatus` 或 runner contract 来承载 queued 状态；仅增加 execution 前的 host-neutral `onJobCreated` handoff，让 application 在 provider dispatch 前绑定 durable `executionJobId`。 |
