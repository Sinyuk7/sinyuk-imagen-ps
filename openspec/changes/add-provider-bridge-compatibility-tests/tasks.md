...
- [ ] 1.1 检查并补齐 `packages/workflows` 的 `vitest.config.ts` 与 `tsconfig.json`，将跨包依赖（`@imagen-ps/core-engine`、`@imagen-ps/providers`）解析到 workspace 源码入口，而不是 `dist`
- [ ] 1.2 在 `packages/workflows/tests/` 下创建 `fixtures.ts`（或 `test-utils.ts`），提供可复用的辅助函数：
...
- [ ] 1.3 运行 `pnpm --filter @imagen-ps/workflows test`，确认现有 `builtins.test.ts` 仍全部通过

## 2. 边界输入与 deep-freeze 兼容性测试

- [ ] 2.1 在 `packages/workflows/tests/` 下创建 `cross-package-compat.test.ts`
- [ ] 2.2 编写 `provider-generate` 边界输入测试：
  - 缺失可选字段（`providerOptions`、`output` 未提供）时，workflow shape 仍合法
  - job input 包含额外字段时，验证是否被正确透传或忽略
- [ ] 2.3 编写 `provider-edit` 边界输入测试：
  - 空 `inputAssets` 数组
  - 缺失可选字段时，workflow shape 仍合法
- [ ] 2.4 编写 deep-freeze / immutability 跨包测试：
...
- [ ] 4.1 使用 `openai-compatible` provider 构造 `ProviderDispatchAdapter`
- [ ] 4.2 编写 `provider-generate` 与 `openai-compatible` adapter 的最小 happy path 测试：
...
- [ ] 4.3 编写 `provider-edit` 与 `openai-compatible` adapter 的边界拒绝测试：
  - 验证当前 `openai-compatible` provider 会在 transport 前拒绝 `edit` 调用
  - 将其记录为当前兼容性边界，而不是成功路径

## 5. 验证、记录与收尾

- [ ] 5.1 运行完整测试套件 `pnpm --filter @imagen-ps/workflows test`，确认所有新增与既有测试通过
- [ ] 5.2 若测试暴露出现有契约与真实 provider 不兼容的问题，记录到 `packages/workflows/STATUS.md` 或 `packages/workflows/OPEN_ITEMS.md`
- [ ] 5.3 更新 `packages/workflows/STATUS.md` 中 §2 Open Questions / Risks 的验证缺口状态
- [ ] 5.4 检查新增测试代码的 JSDoc 与注释，确保关键 helper 与构造器有简短说明
