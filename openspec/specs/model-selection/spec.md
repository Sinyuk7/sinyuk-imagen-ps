...
## Purpose
定义 provider dispatch 时 model 的三级优先级选择语义，确保 job input explicit model、provider profile defaultModel、provider implementation fallback default 按优先级生效，并支持 profile 更新后立即生效。
